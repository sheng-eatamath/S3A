...
class Transformer_VTMIM(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None):
        super().__init__()
        self.width = width
        self.layers = layers
...
class VisionTransformer_MIM(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, mask: bool):
        ...
        self.transformer = Transformer_VTMIM(width, layers, heads)
        self.heads = heads
    ...
    def forward(self, x: torch.Tensor, mask=None, return_features=False):
        ...
        if mask is not None:
            ...
            mask_token_embedding = self.mask_token_embedding.expand(x.shape[0], x.shape[1], -1) ### B x L x D
            w = mask.flatten(1).unsqueeze(-1).type_as(mask_token_embedding) ### B x W x H -> B x WH x 1
            x = x * (1 - w) + mask_token_embedding * w ### masking
        ...
        if mask is not None:
            ...
            prj_features = self.projection_head(x_cls).float()
            prj_features = F.normalize(prj_features, dim=-1)
            x_cls = F.normalize(x_cls @ self.proj,dim=-1)
            x_proj = x_cls
            logits = 100 * x_cls @ self.classifier.float()
            ...
            loss_align = torch.sum((x_mask - x_cls.unsqueeze(1)).pow(2), dim=-1, keepdim=True) ### compute mse loss
            loss_align = loss_align[w.bool()].view(w.size(0), -1) ### select masked patches
      
            mask = mask.repeat_interleave(self.patch_size, 1).repeat_interleave(self.patch_size, 2).unsqueeze(1).contiguous() ### to pixel scale
            if return_features:
                return logits, x_proj, x_rec, loss_align, mask, prj_features
            return logits, x_rec, loss_align, mask, prj_features
        else:
            x_cls = self.ln_post(x[:, 0, :])        
            prj_features = self.projection_head(x_cls).float()
            prj_features = F.normalize(prj_features, dim=-1)
            x_proj = F.normalize(x_cls @ self.proj, dim=-1)        
            logits = 100 * x_proj @ self.classifier.float()
            if return_features:
                return logits, x_proj, prj_features
            return logits, prj_features
    
    def extract_features(self, x, mask=False):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
...
class CLIP(nn.Module):
    ...
    def extract_vfeatures(self, image, **kwargs):
        return self.visual.extract_features(image.type(self.dtype), **kwargs)
