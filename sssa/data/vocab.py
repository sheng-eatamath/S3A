...
def get_vocab_from_file():
    ...
    """
    with open(f'{HOME}/dataset/wordnet_nouns_with_synset_4.pkl', 'rb') as f:
        vocab = pickle.load(f)
    return vocab
...
def read_imagenet21k_classes():
    with open(f'{HOME}/dataset/imagenet21k/imagenet21k_wordnet_ids.txt', 'r') as f:
        data = f.read()
...
def get_classifier(args):
    ...
    if args.vocab_name == 'in21k':
        classifier = torch.load(f'{HOME}/sssa/ipynb/cache/wordnet_classifier_in21k_word.pth')
    elif args.vocab_name == 'in21k-L':
        classifier = torch.load(f'{HOME}/sssa/ipynb/cache/wordnet_classifier_in21k_word_L.pth')
    else:
        raise NotImplementedError()
