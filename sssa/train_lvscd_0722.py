...
# from data.imagenet_datasets_pll_0515 import get_datasets_oszsl
# from data.imagenet_datasets_namevocab import get_datasets_oszsl
from data.imagenet_datasets import get_datasets_oszsl
from engine_self_training_lvscd_0722 import *
from data.vocab import get_classifier_concat2 as get_classifier_wordnet
...
from my_util_package import memory_bank
...
def get_args():
    ...
    # [NOTE: deprecated] distributed training parameters
    ...
    parser.add_argument('--uk', type=str2bool, default=False)
    parser.add_argument('--estimate_k', type=int, default=None)
    parser.add_argument('--total_iter', type=int, default=-1)
    parser.add_argument('--total_iter_min_len_loader', type=str2bool, default=True)
    parser.add_argument('--n_step_scd', type=int, default=3)
    parser.add_argument('--w_loss_st', type=float, default=1.0)
    parser.add_argument('--use_resume', type=str2bool, default=False)
    parser.add_argument('--resume_ckpt', type=str, default=None)
    
    ### dataset
    parser.add_argument('--dataset', default='imagenet', type=str, help='dataset name')
    parser.add_argument('--label_dataset', type=str2bool, default=False)
    parser.add_argument('--n_sampled_classes', type=int, default=100)
    parser.add_argument('--nb_classes', default=0, type=int, help='number of the classification types')
    parser.add_argument('--vocab_name', type=str, default='in21k')
    
    """ METHOD """
    # ### contrastive learning
    # parser.add_argument('--use_cl', type=str2bool, default=False) ### not validated yet
    # parser.add_argument('--tau_cl_self', type=float, default=0.01)
    # parser.add_argument('--tau_cl_sup', type=float, default=0.01)
    # parser.add_argument('--w_cl', type=float, default=1.0)
    parser.add_argument('--use_kmeans', type=str2bool, default=False)
    
    
    # ### CHATGPT
    parser.add_argument('--use_chatgpt', type=str2bool, default=False)
    parser.add_argument('--epoch_chatgpt_warmup', type=int, default=2)
    parser.add_argument('--llm_method', type=str, default='vde', choices=['vde', 'mag-vote', 'mag-logic'])
    # parser.add_argument('--use_llm_captions', type=str2bool, default=False)
    # parser.add_argument('--w_llm_captions', type=float, default=0.0)
    # parser.add_argument('--th_llm_captions', type=float, default=0.3)
    # parser.add_argument('--tau_llm_captions', type=float, default=0.1)
    parser.add_argument('--w_ssl_clu', type=float, default=0.25)
    parser.add_argument('--suffix', type=str, default="")
    
    ### epoch clustering
    # parser.add_argument('--fpath_preclustering', type=str, default=None)
    parser.add_argument('--use_epoch_clustering', type=str2bool, default=False) ### update clustering
    parser.add_argument('--start_epoch_clustering', type=int, default=0)
    parser.add_argument('--per_epoch_clustering', type=int, default=1)
...
def main(args):
    ...
    dataset_train = get_datasets_oszsl(args, vocab=vocab, is_train=True, transform=transform_train)
    ...
    if args.use_resume:
        ...
        # if 'epoch_clustering' in ckpt.keys():
        #     epoch_clustering = ckpt['epoch_clustering']
        # else:
        #     epoch_clustering = None
        ...
        data = load_chatgpt_precomputed_results(args, 'vde')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        condition_update = (args.use_epoch_clustering) and (epoch>=args.start_epoch_clustering) \
            and ((epoch%args.per_epoch_clustering)==(args.per_epoch_clustering-1))
        ...
        if args.use_chatgpt:
            ### warmup epoch
            if epoch < args.epoch_chatgpt_warmup:
            ...
            if epoch==0:
                epoch_clustering = np.load(f'/home/sheng/sssa/ipynb/cache/cluster/kmeans-{args.dataset}{"-"+args.suffix if len(args.suffix) else ""}.npy')
            current_epoch_clustering, cluster_ind_voc = ssl_clustering_func(
                args, model_ema.ema, None, data_loader_trainval, 
                epoch=epoch, log_writer=log_writer, 
                pred_kmeans_t=epoch_clustering,
                return_details=False,
                load_chatgpt=False,
                )
        
            
        ### update @epoch_clustering
        if condition_update:
            epoch_clustering = current_epoch_clustering
        else:
            pass 
        if hasattr(dataset_train, 'dataset'):
            ...
            dataset_train.dataset.ssl_cluster = a
            # dataset_trainval.dataset.ssl_cluster = a
        else:
            dataset_train.ssl_cluster = cluster_ind_voc.cpu()
            dataset_trainval.ssl_cluster = cluster_ind_voc.cpu()
            
            
        train_stats = train_one_epoch_v2_dp(
        ...
        if args.output_dir and (epoch + 1) % args.eval_freq == 0:
            ...
            if (epoch % 5 == 4) and (args.use_kmeans):
                kmeans = KMeans(n_clusters=dataset_trainval.num_classes, random_state=0, n_init=10, max_iter=1000, verbose=1)
                kmeans.fit(all_prj_features_ema)
                epoch_clustering = torch.from_numpy(kmeans.labels_).clone()
                # cluster_acc(y_true, y_pred=epoch_clustering)
    ...
    # print('TRAINVAL results')
    # if args.use_chatgpt:
    #     model.eval()
    #     current_epoch_clustering, cluster_ind_voc, _, _ = compute_ssl_clustering(
    #         args, model, None, data_loader_trainval, 
    #         epoch=31, log_writer=log_writer, 
    #         pred_kmeans_t=epoch_clustering,
    #         return_details=False,
    #         load_chatgpt=True,
    #         )
