...
for dataset_name in ['make_nonliving26']:
    print('='*30)
    print(dataset_name)
    ...
    for suffix in ['pos=0.8']:
        print('-'*10)
        print(suffix)
        pred_kmeans = torch.from_numpy(np.load(f'./cache/cluster/kmeans-{args.dataset}-{suffix}.npy'))
        ...
        try:
            with open(f'./cache/openai/topk=1-visual-inov-template=9_1-k_1={k_1}-repeat={n_repeat}-data={args.dataset}-suffix={suffix}.pkl', 'rb') as f:
        ...
        if not skipdump:
            with open(f'./cache/openai/topk=1-visual-inov-template=9_1-k_1={k_1}-repeat={n_repeat}-data={args.dataset}-suffix={suffix}.pkl', 'wb') as f:
        ...
        mapping_classnames = lambda x: list(map(lambda item: vocab.mapping_idx_names[item], x))
        classnames_pred = mapping_classnames(cluster_ind_voc.cpu().numpy())
        classnames_gt = mapping_classnames(all_gt_voc.cpu().numpy())

        from my_util_package.evaluate import measure_similarity_bert
        score = measure_similarity_bert(vocab, classnames_pred, classnames_gt, device=args.device)
        print(f'score=', score)
        ...
        if args.arch == 'ViT-B/16':
            torch.save(result_data, f'/home/sheng/sssa/ipynb/cache/training/topk=1-cache-inov-{args.dataset}-clip-chatgpt-suffix={suffix}.pth')
        elif args.arch == 'ViT-L/14':
            torch.save(result_data, f'/home/sheng/sssa/ipynb/cache/training/topk=1-cache-inov-{args.dataset}-clip-chatgpt-L-suffix={suffix}.pth')
        else:
