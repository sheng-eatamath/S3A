    ...
    if pred_kmeans_t is None:
        if load_chatgpt:
            if args.fpath_preclustering_chatgpt is None:
                pred_kmeans = np.load(f'{HOME}/sssa/ipynb/cache/cluster/topk=1-cache-inov-{args.dataset}-clip-chatgpt.pth.npy')
            else:
            ...
            if args.fpath_preclustering is None:
                pred_kmeans = np.load(f'{HOME}/sssa/ipynb/cache/cluster/kmeans-{args.dataset}.npy')
            else:
...
def load_chatgpt_precomputed_results(args, llm_method):
    ...
    if llm_method == 'vde':
        if args.clip_model == 'ViT-B/16':
            if args.uk:
                data = torch.load(f'{HOME}/sssa/ipynb/cache/training/topk=1-cache-inov-{args.dataset}-uk={str(args.estimate_k)}-clip-chatgpt.pth') 
            else:
                if len(args.suffix):
                    data = torch.load(f'{HOME}/sssa/ipynb/cache/training/topk=1-cache-inov-{args.dataset}-clip-chatgpt-suffix={args.suffix}.pth')
                else:
                    data = torch.load(f'{HOME}/sssa/ipynb/cache/training/topk=1-cache-inov-{args.dataset}-clip-chatgpt.pth')
        elif args.clip_model == 'ViT-L/14':
            data = torch.load(f'{HOME}/sssa/ipynb/cache/training/topk=1-cache-inov-{args.dataset}-clip-chatgpt-L.pth')
        else:
            raise NotImplementedError()
